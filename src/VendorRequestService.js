var MidtsVendorRequestService = (function () {
  ...
  function renderRequestSetup(e) {
    ...
    if (String(leadResult.lead['Lifecycle Status'] || '') !== 'Vendor Pricing') {
      return htmlPage_('Vendor request unavailable', '<p>This lead is not ready for partner assessment and vendor pricing.</p>');
    }
    ...
    var body = [
      ...
      '<button type="submit">Send assessment request</button>',
      '</form>'
    ].join('');
  ...
  function renderPartnerAssessmentForm(e) {
    ...
    var lead = leadResult.lead;
    var alreadySubmitted = String(request.record['Technical Review ID'] || '').trim();
    if (alreadySubmitted) {
    ...
    var body = [
      ...
      '<dl><dt>Project reference</dt><dd>' + escapeHtml_(request.record['Quote Reference'] || request.record['Lead ID']) + '</dd>',
      '<dt>Project</dt><dd>' + escapeHtml_(lead['Project Type'] || '') + '</dd>',
      '<dt>Requirement summary</dt><dd>' + escapeHtml_(lead['Brief Requirement'] || '') + '</dd></dl>',
      ...
      '<form method="post" action="' + escapeHtml_(MidtsConfig.getWebAppUrl()) + '">',
      hiddenInput_('action', 'partnerAssessment'),
  ...
  function handlePartnerAssessmentSubmission(e) {
    ...
    try {
      ...
      if (String(request.record['Technical Review ID'] || '').trim()) {
        return htmlPage_('Assessment already received', '<p>This assessment has already been submitted. No duplicate latest assessment was created.</p>');
      }
      ...
      MidtsLogger.logWebhookAttempt({
        requestId: 'PARTNER-ASSESSMENT-' + request.record['Request ID'],
        outcome: allowPricing ? 'partner_assessment_pricing_ready' : 'partner_assessment_pricing_blocked',
        message: 'Partner assessment received: ' + result.feasibilityStatus,
        payload: { leadId: result.leadId, requestId: request.record['Request ID'], reviewId: result.reviewId, pricingReady: allowPricing },
        submissionId: '',
  ...
  function renderVendorPricingForm(e) {
    ...
    var body = [
      ...
      '<p class="lede">Please provide your commercial response for the approved assessment basis below.</p>',
      '<dl><dt>Reference</dt><dd>' + escapeHtml_(request.record['Quote Reference'] || request.record['Lead ID']) + '</dd>',
      '<dt>Project</dt><dd>' + escapeHtml_(lead['Project Type'] || '') + '</dd>',
      '<dt>Requirement</dt><dd>' + escapeHtml_(lead['Brief Requirement'] || '') + '</dd>',
      '<dt>Partner assessment</dt><dd>' + escapeHtml_(request.record['Partner Assessment Status'] || '') + '</dd></dl>',
      '<p><a class="package" href="' + escapeHtml_(request.record['Vendor Package Link']) + '" target="_blank" rel="noopener">Open vendor-safe package</a></p>',
      '<form method="post" action="' + escapeHtml_(MidtsConfig.getWebAppUrl()) + '">',
  ...
  function handleVendorPricingSubmission(e) {
    ...
    try {
      ...
      MidtsLogger.logWebhookAttempt({
        ...
        payload: { leadId: pricing.leadId, pricingId: pricing.pricingId, vendorRequestId: request.record['Request ID'], technicalReviewId: request.record['Technical Review ID'] },
        submissionId: '',
        email: request.record['Vendor Email'],
        source: 'Vendor Request Service'
      });
  ...
  function sendRequestSetupEmail(leadId) {
    ...
    var body = [
      'The vendor-safe package is ready. Select the partner and send the assessment request.',
      'Vendor pricing remains blocked until the partner assessment is feasible and pricing-ready.',
      '',
      'Lead ID: ' + leadId,
      'Client: ' + (lead['Full Name'] || ''),
      'Project: ' + (lead['Project Type'] || ''),
      '',
      'Send partner assessment request: ' + url
    ].join('\n');
  ...
  function buildRequestSetupUrl(leadId) {
    return MidtsConfig.getWebAppUrl() + '?action=vendorRequestSetup&leadId=' + encodeURIComponent(leadId) + '&token=' + encodeURIComponent(MidtsConfig.getDecisionToken());
  }
  ...
  function createAndSendRequest_(input) {
    ...
    try {
      ...
      var assessmentUrl = buildPartnerAssessmentUrl_(requestId, rawToken);
      var request = {
        ...
        'Partner Assessment Status': 'Requested',
        'Partner Assessment Link': '',
        'Pricing Readiness': 'No',
        'Submitted At': '',
        'Vendor Cost': '',
        'Vendor Currency': '',
        'Lead Time': '',
      ...
      var emailResult = sendVendorRequestEmail_(request, leadResult.lead, assessmentUrl);
      ...
      if (!emailResult.ok) {
        ...
        return { ok: false, message: 'Partner assessment request could not be emailed: ' + emailResult.message };
      }
  ...
  function sendVendorRequestEmail_(request, lead, assessmentUrl) {
    ...
    var body = [
      ...
      '',
      'MIDTS requests your Partner Technical Assessment for the vendor-safe package linked below.',
      'Please complete the assessment before submitting any commercial pricing.',
      '',
      'Package: ' + request['Vendor Package Link'],
      'Submit assessment: ' + assessmentUrl,
      '',
      'Do not include MIDTS margin, client selling price, or unrelated client information in your response.',
      '',
      'MIDTS'
    ].join('\n');
    try {
      MailApp.sendEmail({
        ...
        htmlBody: [
          ...
          '<p>MIDTS requests your <strong>Partner Technical Assessment</strong> for the vendor-safe package.</p>',
          '<p><a href="' + escapeHtml_(request['Vendor Package Link']) + '">Open vendor-safe package</a></p>',
          '<p><a href="' + escapeHtml_(assessmentUrl) + '" style="display:inline-block;padding:10px 14px;border:1px solid #111;color:#111;text-decoration:none">Submit assessment</a></p>',
          '<p>Please complete the assessment before submitting any commercial pricing.</p>',
          '<p>MIDTS</p></div>'
        ].join('')
      });
  ...
  function sendAssessmentReceivedEmail_(request, assessment, pricingReady) {
    ...
    var body = [
      'Partner Technical Assessment received.',
      '',
      'Lead ID: ' + assessment.leadId,
      'Partner: ' + assessment.reviewerOrganisation,
      'Outcome: ' + assessment.feasibilityStatus,
      'Pricing ready: ' + (pricingReady ? 'Yes' : 'No'),
      'Assessment document: ' + assessment.partnerAssessmentDocumentLink,
      'Workspace: ' + workspaceBase + '/workspace/vendor-request/review?leadId=' + encodeURIComponent(assessment.leadId)
    ].join('\n');
    try {
  ...
  function appendRequest_(request) {
    var sheet = getSheet_();
    ensureHeaders_(sheet);
    var headerMap = sheet.getRange(1, 1, 1, sheet.getLastColumn()).getValues()[0].reduce(function (map, header, index) { map[String(header || '').trim()] = index + 1; return map; }, {});
    var row = [];
    for (var i = 0; i < sheet.getLastColumn(); i += 1) row.push('');
    HEADERS.forEach(function (header) { if (headerMap[header]) row[headerMap[header] - 1] = request[header] === undefined ? '' : request[header]; });
    sheet.appendRow(row);
  }
  ...
  function findOpenRequestForLeadAndVendor_(leadId, vendorEmail) {
    ...
    for (var i = 1; i < values.length; i += 1) {
      ...
      if (sameLead && sameVendor && ['Pending Send', 'Sent', 'Assessment Received'].indexOf(status) !== -1) return true;
    }
    return false;
  }
  ...
  function updateRequest_(result, updates) {
    ...
    Object.keys(updates).forEach(function (header) {
      ...
      result.record[header] = updates[header];
    });
  }

  function buildPartnerAssessmentUrl_(requestId, rawToken) {
    return MidtsConfig.getWebAppUrl() + '?action=partnerAssessment&requestId=' + encodeURIComponent(requestId) + '&token=' + encodeURIComponent(rawToken);
  }

  function buildVendorPricingUrl_(requestId, rawToken) {
    var frontendUrl = MidtsConfig.getScriptProperty('VENDOR_PRICING_FORM_URL');
    if (frontendUrl) return frontendUrl + (frontendUrl.indexOf('?') === -1 ? '?' : '&') + 'requestId=' + encodeURIComponent(requestId) + '&token=' + encodeURIComponent(rawToken);
    return MidtsConfig.getWebAppUrl() + '?action=vendorPricing&requestId=' + encodeURIComponent(requestId) + '&token=' + encodeURIComponent(rawToken);
  }

  function guardCompleteAssessment_(review) {
    var required = ['Technical Review ID', 'Reviewer', 'Reviewer Organisation', 'Reviewer Email', 'Files And Revisions Reviewed', 'Partner Review Package Link', 'Partner Assessment Document Link', 'Feasibility Status', 'Partner Submitted At', 'Review Summary', 'Recommendation'];
    ...
    var feasible = String(review['Feasibility Status'] || '');
    if (feasible !== 'Feasible' && feasible !== 'Feasible with Assumptions') return { ok: false, message: 'Vendor pricing requires a Feasible partner assessment. Current outcome: ' + feasible + '.' };
    return { ok: true };
  }

  function recommendationForFeasibility_(value) {
    var status = String(value || '').trim();
    if (status === 'Feasible' || status === 'Feasible with Assumptions') return 'Qualified';
    if (status === 'Clarification Required' || status === 'Alternative Approach') return 'Needs More Info';
    return 'Not Suitable';
  }
  ...
  function buildPricingNotes_(params) {
    return ['Vendor request: ' + String(params.requestId || ''), 'Vendor reference: ' + String(params.vendorReference || '').trim(), 'Lead time: ' + String(params.leadTime || '').trim(), 'Quote valid until: ' + String(params.quoteValidUntil || '').trim(), 'Exclusions or assumptions: ' + String(params.exclusions || '').trim(), 'Vendor notes: ' + String(params.notes || '').trim()].join('\n');
  }
  ...
  return {
    ...
    renderPartnerAssessmentForm: renderPartnerAssessmentForm,
    handlePartnerAssessmentSubmission: handlePartnerAssessmentSubmission,
