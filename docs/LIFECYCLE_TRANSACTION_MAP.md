...
| 9 | Vendor request / Partner Technical Assessment setup | MIDTS commercial | `NEW-MIDTS/workspace/vendor-request` -> `/workspace/vendor-request/review?leadId=...` | `formStage=vendorRequestSetup`, `action=setupVendorRequest`, `leadId`, `vendorName`, `vendorEmail`, `packageLink`, `pageUrl` | Gateway -> `WebhookRouter.handlePost` -> `VendorRequestService.setupVendorRequest` -> `createAndSendRequest_` | `Vendor Requests`, `Leads`, `Email Logs` | Partner Technical Assessment request email | Yes | Workspace sends the assessment request first. Pricing remains blocked until feasible plus pricing-ready assessment is stored. |
| 10 | Partner Technical Assessment and vendor commercial response | Partner / Vendor | Partner assessment link first; pricing link becomes usable only after feasible/pricing-ready assessment | Assessment: `action=partnerAssessment`, `requestId`, `token`, assessor, organisation, evidence link, feasibility, pricingReady. Pricing: `formStage=vendorPricing`, `action=vendorPricing`, `requestId`, `token`, `vendorCost`, `vendorCurrency`, `leadTime`, `quoteValidUntil`, `vendorReference`, `exclusions`, `notes` | Assessment: `doPost(action=partnerAssessment)` -> `VendorRequestService.handlePartnerAssessmentSubmission` -> `TechnicalReviewService.recordReview`. Pricing: `doPost(action=vendorPricing)` -> `VendorRequestService.handleVendorPricingSubmission` -> `VendorPricingService.recordVendorPricing` | `Vendor Requests`, `Technical Reviews`, `Vendor Pricing`, `Leads`, `Webhook Logs`, `Email Logs` | Assessment received notification; pricing received notification | Yes | Direct pricing is guarded by latest feasible Partner Technical Assessment evidence. Old pricing links cannot bypass assessment readiness. |
...
3. Vendor request setup and Partner Technical Assessment bridge.
   - MIDTS sends assessment request from Workspace.
   - Partner submits assessment evidence through the tokenized assessment form.
   - Vendor pricing remains blocked until feasible and pricing-ready.
4. Margin approval.
